...
export function getPictures(category: string, page = 1): Promise<Image[]> {
  const url =
    `${BASE_URL}?key=${PIXABAY_KEY}&safesearch=true&page=${page}&per_page=25&category=${category}`;
...
export function getVideos(category: string, page = 1): Promise<Video[]> {
  const url =
    `${BASE_URL}videos/?key=${PIXABAY_KEY}&safesearch=true` +
    '&page=${page}&per_page=25&category=${category}';
